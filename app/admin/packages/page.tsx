...
export default async function AdminPackagesPage({
  ...
  return (
    <div>
          ...
          return (
            <Link
              ...
              className={`text-xs px-3 py-1.5 rounded-full border ${
                isActive ? `${f.className} text-white` : "bg-zinc-900 border-zinc-800 text-zinc-400 hover:text-white"
              }`}
            >
              {f.label} ({count})
            </Link>
      ...
      {(pkgs ?? []).length === 0 ? (
        <p className="text-zinc-500 text-sm text-center py-12 bg-zinc-900 border border-zinc-800 rounded-2xl">
          {status === "pending"
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; className: string }> = {
    ...
    delivered: { label: "Entregado", className: "bg-emerald-500/20 text-emerald-300" },
    returned: { label: "Devuelto", className: "bg-amber-500/20 text-amber-300" },
  };
  const m = map[status] ?? { label: status, className: "bg-zinc-700 text-zinc-300" };
