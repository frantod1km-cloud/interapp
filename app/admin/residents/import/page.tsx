...
export default async function ImportResidentsPage({
  ...
  return (
    <div>
      <Link href="/admin/residents" className="text-sm text-zinc-700 hover:text-zinc-700 mb-4 inline-block">
        ← Volver
      </Link>
      ...
      <div className="bg-white border border-zinc-200 rounded-2xl p-4 mb-6 text-sm">
...
28999111,María,López,Lote 18,1133445566`}
      ...
      {sp.error && (
        <div className="bg-rose-700/20 border border-rose-700/40 rounded-2xl p-4 mb-4 text-sm text-rose-700 whitespace-pre-wrap">
      ...
      <form action={importResidentsAction} className="space-y-4">
        <textarea
          ...
          className="w-full bg-white border border-zinc-200 rounded-lg px-4 py-3 font-mono text-sm"
        />
        <div className="flex gap-3">
          <button
            type="submit"
            className="bg-blue-600 hover:bg-blue-500 font-semibold rounded-lg px-6 py-3"
          >
            Importar
          </button>
          <Link
            href="/admin/residents"
            className="bg-zinc-100 hover:bg-zinc-200 font-semibold rounded-lg px-6 py-3"
          >
            Cancelar
          </Link>
        </div>
