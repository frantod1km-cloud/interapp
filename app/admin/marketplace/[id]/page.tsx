...
export default async function AdminListingDetailPage({
  ...
  return (
    <div>
      <Link href="/admin/marketplace" className="text-sm text-zinc-400 hover:text-zinc-400 inline-block mb-4">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden mb-6">
        {listing.photo_url ? (
          ...
          <img src={listing.photo_url} alt="" className="w-full h-48 sm:h-64 object-cover bg-zinc-800" />
        ) : (
        ...
        <div className="p-6">
          <div className="flex items-center gap-2 mb-2 flex-wrap">
            ...
            {!listing.active && (
              <span className="text-xs px-2 py-1 rounded bg-rose-700/20 text-rose-300">Inactivo</span>
            )}
          </div>
          ...
          <div className="flex gap-2 mt-6">
            ...
            <form action={deleteListingAction}>
              ...
              <button className="bg-zinc-800 hover:bg-rose-700 text-sm px-4 py-2 rounded-lg">
                Eliminar
              </button>
            </form>
          </div>
      ...
      {!reservations || reservations.length === 0 ? (
        <p className="text-zinc-400 text-sm bg-zinc-900 border border-zinc-800 rounded-2xl p-6 text-center">
                ...
                return (
                  <tr key={r.id} className="border-t border-zinc-800">
                    ...
                    <td className="px-4 py-3">
                      ...
                      {res?.unit && <span className="text-zinc-400"> · {res.unit}</span>}
                    </td>
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; cls: string }> = {
    pending_payment: { label: "Esperando pago", cls: "bg-amber-500/20 text-amber-300" },
    confirmed: { label: "Confirmada", cls: "bg-emerald-500/20 text-emerald-400" },
    cancelled: { label: "Cancelada", cls: "bg-zinc-700/40 text-zinc-400" },
    completed: { label: "Completada", cls: "bg-sky-500/20 text-sky-300" },
  };
  const m = map[status] ?? { label: status, cls: "bg-zinc-700 text-zinc-400" };
