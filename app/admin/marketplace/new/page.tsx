...
export default async function NewListingPage({
          ...
          {ALL_KINDS.map((k) => {
            ...
            return (
              <Link
                ...
                className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 hover:border-emerald-500/50 transition"
              >
  ...
  return (
    <div>
      <Link href="/admin/marketplace/new" className="text-sm text-zinc-500 hover:text-zinc-300 inline-block mb-4">
        ← Cambiar tipo
      </Link>
      <h1 className="text-2xl font-bold mb-2">
        {KIND_META[selectedKind].emoji} Nuevo {KIND_META[selectedKind].label.toLowerCase()}
      </h1>
      <p className="text-zinc-400 text-sm mb-6">{KIND_META[selectedKind].description}</p>
