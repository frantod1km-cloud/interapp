    ...
    <form action={action} className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <div className="sm:col-span-2">
          <label className="block text-sm mb-1 text-zinc-400">Nombre</label>
          <input
            name="name"
            required
        ...
        <div>
          <label className="block text-sm mb-1 text-zinc-400">Precio (ARS)</label>
          <input
            name="price_ars"
            type="number"
            min={0}
      ...
      {/* Foto */}
      <div>
        <label className="block text-sm mb-1 text-zinc-400">Foto</label>
        <div className="flex items-center gap-3">
          {photoUrl ? (
            <>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={photoUrl} alt="" className="w-24 h-24 rounded-lg object-cover bg-zinc-800" />
              <button
                type="button"
                onClick={() => {
                  setPhotoUrl("");
      ...
      {kind === "space" && (
        <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-4 space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            <div>
              <label className="block text-xs text-zinc-400 mb-1">Duración del slot (min)</label>
              <input
                name="slot_minutes"
                type="number"
                min={15}
            ...
            <div>
              <label className="block text-xs text-zinc-400 mb-1">Cupo simultáneo</label>
              <input
                name="max_concurrent"
                type="number"
                min={1}
                ...
                className="w-full bg-zinc-900 border border-zinc-800 rounded px-3 py-2 text-sm"
              />
              <p className="text-xs text-zinc-600 mt-0.5">1 = uso exclusivo</p>
            </div>
            <div>
              <label className="block text-xs text-zinc-400 mb-1">Días para reservar</label>
              <input
                name="advance_days"
                type="number"
                min={1}
            ...
            <div>
              <label className="block text-xs text-zinc-400 mb-1">Abre a las</label>
              <select
                name="open_hour"
            ...
            <div>
              <label className="block text-xs text-zinc-400 mb-1">Cierra a las</label>
              <select
                name="close_hour"
      ...
      {kind === "event" && (
        <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div>
              <label className="block text-xs text-zinc-400 mb-1">Fin (opcional)</label>
              <input
                name="event_ends_at"
                type="datetime-local"
                defaultValue={
            ...
            <div>
              <label className="block text-xs text-zinc-400 mb-1">Cupo total (opcional)</label>
              <input
                name="event_capacity"
                type="number"
                min={1}
      ...
      {kind === "membership" && (
        <div className="bg-zinc-950 border border-zinc-800 rounded-xl p-4 space-y-3">
          ...
          <div>
            <label className="block text-xs text-zinc-400 mb-1">Duración del período</label>
            <select
