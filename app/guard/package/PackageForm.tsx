  ...
  return (
    <form action={createPackageAction} onSubmit={onSubmit} className="space-y-4">
      ...
      <div>
        ...
        {selected ? (
          ...
          <>
            ...
            />
            {filtered.length > 0 && (
              <ul className="mt-2 bg-zinc-900 border border-zinc-800 rounded-lg divide-y divide-zinc-800 max-h-72 overflow-y-auto">
                {filtered.map((r) => {
                  ...
                  return (
                    <li key={r.id}>
                      <button
                        ...
                        className="w-full text-left px-4 py-2 hover:bg-zinc-900 border border-zinc-800 flex items-center justify-between gap-3"
                      >
                        <div>
                        ...
                        <span className="text-xs text-zinc-400">Elegir →</span>
                      </button>
                    </li>
                  );
                })}
            ...
            {q && filtered.length === 0 && (
              <p className="text-xs text-zinc-400 mt-2">Sin resultados.</p>
            )}
          </>
        )}
      ...
      <div>
        ...
        {photoError && <p className="text-rose-300 text-xs mt-1">{photoError}</p>}
