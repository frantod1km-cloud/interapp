...
function expiryStatus(expiresAt: string | null): ExpiryStatus | null {
  ...
  if (days < 0) {
    return {
      ...
      className: "bg-rose-700/20 text-rose-700 border border-rose-600/40",
    };
  }
  if (days <= 7) {
    return {
      label: `Vence en ${days === 0 ? "hoy" : `${days}d`} (${d.toLocaleDateString("es-AR")})`,
      className: "bg-amber-600/20 text-amber-700 border border-amber-600/40",
    };
  }
  return {
    label: `Vence ${d.toLocaleDateString("es-AR")}`,
    className: "bg-zinc-200/40 text-zinc-700",
  };
}
...
export default async function ResidentPeoplePage({
  ...
  return (
    <div>
      ...
      <details className="bg-white border border-zinc-200 rounded-2xl mb-6 group">
        <summary className="cursor-pointer p-4 font-semibold flex items-center justify-between list-none">
          <span>+ Agregar persona</span>
          <span className="text-emerald-700 text-xl transition-transform group-open:rotate-45">+</span>
        </summary>
        <form action={addPersonAction} className="p-4 pt-0 space-y-3 border-t border-zinc-200">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label className="block text-xs text-zinc-700 mb-1">Categoría</label>
              <select name="kind" defaultValue="domestic" required className="w-full bg-white rounded px-3 py-2 border border-zinc-200">
            ...
            <div>
              ...
              <input name="dni" required inputMode="numeric" className="w-full bg-white rounded px-3 py-2 border border-zinc-200" />
            </div>
            <div>
              <label className="block text-xs text-zinc-700 mb-1">Nombre</label>
              <input name="first_name" required className="w-full bg-white rounded px-3 py-2 border border-zinc-200" />
            </div>
            <div>
              <label className="block text-xs text-zinc-700 mb-1">Apellido</label>
              <input name="last_name" required className="w-full bg-white rounded px-3 py-2 border border-zinc-200" />
            </div>
            <div className="sm:col-span-2">
              <label className="block text-xs text-zinc-700 mb-1">
                Fecha de vencimiento del acceso <span className="text-zinc-700">(opcional)</span>
              </label>
              <input
                type="date"
                name="access_expires_at"
                min={new Date().toISOString().slice(0, 10)}
                className="w-full bg-white rounded px-3 py-2 border border-zinc-200"
              />
      ...
      <div className="space-y-3">
        ...
        {(people ?? []).map((p) => {
          ...
          return (
            <div key={p.id} className={`bg-white border rounded-2xl p-4 ${p.active ? "border-zinc-200" : "border-zinc-200 opacity-60"}`}>
              <div className="flex items-start justify-between flex-wrap gap-3 mb-3">
                <div className="min-w-0">
                  <div className="flex items-center gap-2 mb-1 flex-wrap">
                    ...
                    {!p.active && (
                      <span className="text-xs bg-zinc-200/40 text-zinc-700 px-2 py-0.5 rounded">
                        Inactivo
                      </span>
                    )}
                    {exp && (
                  ...
                  <div className="text-sm text-zinc-700 tabular-nums">DNI {formatDni(p.dni)}</div>
                  {rule ? (
                    <div className="text-xs text-emerald-700 mt-1">⏱️ {describeRule(rule)}</div>
                  ) : (
                    <div className="text-xs text-zinc-700 mt-1">Sin restricción horaria</div>
                  )}
                </div>
